...
QVariant TransferListModel::data(const QModelIndex& index, int role) const
{
    ...
    switch (role)
    {
        ...
        case ProgressRole:
            // A finished row reads 1.0 even when totalBytes stayed 0: an empty file
            // never reports a byte, and a progress bar left at 0 on a row labelled
            // Completed reads as a stalled transfer.
