...
void MegaSdkClient::getRootChildren(std::function<void(Result<std::vector<FileEntry>>)> onDone)
{
    ...
    if (!root)
    {
        onDone(Result<std::vector<FileEntry>>::fail("No root node (not logged in / nodes not fetched)"));
        return;
    }

    std::unique_ptr<mega::MegaNodeList> children(mApi->getChildren(root.get()));
    ...
    onDone(Result<std::vector<FileEntry>>::ok(std::move(entries)));
}
