...
**最優先は短さ。** リリース文はコミットログの要約ではなく、「この版で何が変わったか」を
訪問者が数秒で掴むためのもの。**1 サイクル = 1 行ではない。** 出荷された項目を並べると
必ず長すぎるので、まず**この版の主題を 1 つ決め**、他をその下に畳めないか考えてから書く。
...
- **一つの機能に付随する細部は、その機能の 1 行に畳む。** 主題となる機能を足したとき、
  ロードマップ上は別項目でも、ユーザーから見て**その機能にあって当然のもの**は独立した
  行を立てない。書くのは「何ができるようになったか」だけで、その中の操作は書かない。
  `v0.2.0` が手本: 出荷 12 件・機能追加 11 件あったが、`### New features` は
  「アプリ内ビューア。ダブルクリックで独立ウィンドウに開く——画像・動画・音声・PDF」の
  **1 行だけ**にした。音量・ミュート・ループ再生・ウィンドウにフィット・前後の画像へ移動・
  複数ウィンドウは、いずれもビューアにあって当然なので 1 行も割いていない。
  **どの形式が開けるかだけは書く**——そこは読み手が事前に知りようがないから。
- **`### Improvements` は「元からあった機能がこう良くなった」だけ。** 今回入った機能の
  一部を、独立項目だったからといってここに置かない——読み手には前からあった機能の改善に
  見えてしまう。主題の機能の内側の話なら New features の 1 行に畳むか、落とす。
  節ごと空になるならその見出しを置かない（`v0.2.0` は Improvements 無しで出した）。
- **ユーザーが気づかないような細かい修正は 1 行にまとめる**——"Fixed several other minor
  issues."。一つ一つ並べない。**本当にそれが有るときだけ書く**（`v0.2.0` は不具合が
  1 件だけだったので、この行は置かなかった）。
...
- 冒頭 1 行は「何のリリースか」＝主題の機能を一言で。初回リリースからの差分が大きいときだけ
  2〜3 行の要約を足す。

目安として、**箇条書きは全部で 5 行を超えたら畳み方を疑う**。`v0.2.0` は 2 行で足りた。

`build/release-notes-X.Y.Z.md` に書く（`/build*/` は gitignore 済み＝コミットされない）。
**書いた文面は毎回そのままターミナルの本文に全文を出す**——ファイルへ書いただけでは人間に
見えない。直したときも**省略せず全文を出し直し**、何を変えたかを 1 行添える（差分だけ見せて
`AskUserQuestion` を出すと、承認する側に判断材料が無い）。そのうえで **`AskUserQuestion` で
「この版・この文面で master へ進む / 直す / 中止」**を聞く。直すと言われたら直してもう一度
聞く。**この承認は版も兼ねる**——版を変えると言われたら 1. の版上げコミットを
`git commit --amend` で作り直し、zip 名もタグもそこから派生するので他に直す場所は無い。
...
- リリース文に**出荷された項目を 1 行ずつ並べない**（2. の畳み方が全部無駄になる）。
