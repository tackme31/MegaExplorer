...
`drive`（実マウス・キーボードの乗っ取り）は、**叩く前に自分で状況を確かめる**:

```
d="${LOCALAPPDATA//\\//}/MegaExplorerLoop"; now=$(date +%s); r=skip
[ -f "$d/last-user-prompt" ] && [ $(( now - $(tr -dc 0-9 < "$d/last-user-prompt") )) -lt 600 ] && r=attended
[ -f "$d/drive-allowed" ]    && [ "$now" -lt "$(tr -dc 0-9 < "$d/drive-allowed")" ]           && r=flag
echo $r
```

| 結果 | 意味 | すること |
| --- | --- | --- |
| `flag` | `drive ON 6h/8h/12h` のショートカットが期限付きフラグを置いている | そのまま実行してよい。フックは尋ねない |
| `attended` | 10 分以内に人間が入力している＝**席にいる** | **実行してよい。** フックが許可を求め、承認されればこのセッションでは以降尋ねられない |
| `skip` | フラグも無く、人間も 10 分以上黙っている＝**無人** | **`drive` を呼ばない。** 確かめたかったことを「人間に確認してほしい観点」へ回す |

**`skip` のときだけ呼んではいけない。** 無人の周が許可プロンプトに当たると誰も答えないまま止まり、
次の cron 発火も来ない。逆に **`attended` で skip するのは誤り**——人が見ている周まで確認を捨てる
ことになる。判定と実行の間に離席した場合は、フックが `ask` ではなく拒否を返すので固まらない
（`scripts/drive_gate_hook.sh`）。

最終入力時刻 `last-user-prompt` は `scripts/away_notify_hook.sh` の `UserPromptSubmit` 側が刻んで
いる。10 分という窓もそちらと同じ値で、「席にいるか」を判定する信号はこのリポジトリに 1 つだけ。
...
- **`drive`** — 事前判定の結果（`flag` / `attended` / `skip`）と、使ったなら何を操作したか
- **Serena** — 通常モードか縮退モードか
