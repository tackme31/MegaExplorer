...
**`drive` で開いた直後の新しいウィンドウには、キー入力が届かない。** 合成入力のときだけ起きる現象で、
手で開けば届く。キー操作を確かめるときは、先にそのウィンドウを 1 回クリックする。不具合として起票しない。
...
3 つに分ける。3. の検証や `drive` の実機確認で気づいたことも、ここで同じように振り分ける:

1. **今サイクルで直す** — 明確な誤りで、直しても項目の範囲を出ないもの。**今周の項目の実装漏れ
   （作った機能が意図どおり動かない）は、直すのが複雑でなければここに入れ、別件として起票しない。**
   直したら 3. をやり直す（`drive` で確かめ直すなら許可も取り直す）。
2. **ROADMAP へ回す** — 正しい指摘だが別件のもの。今周の実装漏れでも、直すのが複雑なもの
   （原因が特定できない、項目の範囲を越えて別の機能に手が入る、設計からやり直す必要がある）は
   ここに入れる。**種類**は運用ルール 6. で決め（既に実装した機能が
...
**ROADMAP / REQUESTS を触った周は、コミット前に必ず差分を検算する:**

```
git diff -U0 docs/ROADMAP.md docs/REQUESTS.md
```

**この周が意図して足した行・消した行・書き換えた行だけが出ていることを確認する。** それ以外の
行が動いていたら意図しない巻き添えなので、`git checkout HEAD -- <file>` で戻し、行単位の編集で
やり直す。直せなければ**コミットせずに中止して報告する**。
...
- **意図しない挙動を機能追加として登録しない。** 今周の項目の実装漏れは、複雑でなければその周で
  直す（4-3）。直さないものは、実装・検証・実機確認・レビューのどこで気づいたものでも
  **種類「不具合」**で、再現手順と気づいた周を添えて起票する
...
- **「見送り (blocked)」節を消さない・書き換えない。** 着手しないだけでなく、節ごと・行ごと
  落とさない。ここが製品スコープの歯止めになっている。
