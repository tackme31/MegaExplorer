...
**1 回の許可はその確認内容の範囲で使い回す。** 状態が 2 つあって `drive` を 2 回叩くなら、それは 1 つの
許可で足りる。レビュー修正後の再確認や別機能の確認は**別の粒度**なので改めて取る。基準は「許可を
取ったときの説明文で、これから叩く `drive` を説明できるか」——説明が要る変化が起きたら新しい許可。
