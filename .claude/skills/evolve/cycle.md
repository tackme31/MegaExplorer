...
2. **ROADMAP へ回す** — 正しい指摘だが別件。行き先は `ROADMAP.md` の運用ルール 6. で決める:
   **既に実装した機能が意図どおり動いていない**なら「バグ修正（最優先）」、**意図どおりだが
   足りない**だけなら優先度セクション。「動きはするので後回しでよい」は優先度セクションへ
   逃がす理由にならない。
...
- **意図しない挙動を機能追加として登録しない。** 実装・検証・実機確認・レビューのどこで気づいた
  ものでも、その周で直さないなら「バグ修正（最優先）」へ、再現手順と気づいた周を添えて起票する
  （`ROADMAP.md` の運用ルール 7.）。原因が未特定でも起票する。
