...
int main(int argc, char* argv[])
{
    ...
    // Windows asks the window for a small and a large icon separately (title
    // bar / Alt-Tab vs. taskbar button), scaled by the monitor's DPI, so one
    // pixmap would be resampled for most of them -- hence every size. The .exe
    // resource in resources/appicon.rc is the other half: it is what Explorer
    // and a pinned taskbar entry read, before the process exists.
    {
        QIcon windowIcon;
