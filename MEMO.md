...
各フェーズは「その時点で単体で動作確認できる状態」を目指し、下位レイヤーから積み上げる。MVP(フェーズ0〜5)を安定させてから、フェーズ6以降の拡張に着手する。

1. **フェーズ0**: MEGA SDKのビルド確認、ログイン・fetchNodesのCLI疎通確認
2. **フェーズ1**: 最小限のファイル一覧表示(サムネイル無し)
3. **フェーズ2**: サムネイル取得・グリッド表示
4. **フェーズ3**: 検索機能
5. **フェーズ4**: DL→既定アプリ起動、アップロード、進捗UI
6. **フェーズ5**: ローカルキャッシュ、フォルダオープン時のバックグラウンド更新、エラーハンドリング全般 ← **ここまででMVP**
7. **フェーズ6(将来拡張)**: リモート変更のリアルタイム反映(機能一覧 7. を参照)。フォルダオープン時更新の上に追加するだけなので、既存構造への影響は小さい
8. **フェーズ7以降(未確定)**: 必要になれば都度検討。現時点ではローカルとの双方向フル同期は対象外
