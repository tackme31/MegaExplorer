...
終了時に `logout()` を呼ぶ（`whoami` を除く）。DB 名はセッション ID から作られるので実行のたびに
別名になり、`MegaApi::logout` だけが消してくれる `removeCaches()` を通さないと 1 回あたり約 1.2MB
の DB とサーバ側のセッションが残り続ける。ベストエフォートで、失敗してもコマンド本来の終了コードは
変えない。
...
```
$ megatool whoami
env    : test-account@example.com
session: test-account@example.com
match  : yes
```
