...
**R1–R7 全スコープ完了（2026-08-08）。** 以降このファイルは実施ログの参照先で、計画としては閉じて
いる。新しい整理項目が出たら「持ち越し」節ではなく、その時点で新しい文書を立てること。

ただし**持ち越し節には意図的に閉じていない 3 件が残っている** —— 配布パイプライン（`windeployqt`/
CPack/Release プリセットがそもそも無い）、`NotificationController` の `Unknown` 経路に残る SDK 生英文
（R3-4 が意図的に残した形で、テストもその前提で書かれている）、デリゲート側のドラッグ重複
（R6 範囲外と判断、下の R6 節末尾）。**どれもリファクタリングの積み残しではなく、スコープ外と
判断した結果**なので、着手するなら独立した項目として立てること。
...
- **対応（2026-08-08）**: 決定どおり (c)。製品コードは 2 つのデストラクタのコメントのみ、
  `.cpp` の実行文は 1 行も変えていない。`ThreadedDeliveryTest` に
  `ClearingAnObserverStopsOnlyTheDeliveriesThatStartAfterIt` を 1 件追加。
  実施して分かったこと:
  - **「観測者を外せば安全」と「shutdown() が停止点」は別々の主張で、前者だけがテストで固定
    できる**。後者が守る窓（ロック下でコピー済みのコピーが呼ばれる区間）は、破れたときの挙動が
    解放済みメモリへの呼び出しなので、**アサートできる観測点が原理的に存在しない**
    （Windows に TSan がないという既知の制約以前の問題）。そこで新テストは
    `setOnProgress(nullptr)` の**前後で 1 回ずつ**ワーカから progress を流し、後者が観測者に
    届かないことだけを固定する。テスト観測者はコントローラではなく**テスト自身のラムダ**に
    した — コントローラを破棄した後に届く呼び出しは、届いたかどうかを観測する手段がない。
  - コメントには **「これはこのクラスの契約ではなく現在の形（アプリ寿命 1 個）の契約である」**
    と、**「per-tab 化したら observer を `weak_ptr` に変える」** の 2 点を明記した。R5-9 の
    実体はこの 2 行で、`shutdown()` の説明はその根拠にすぎない。
  - `UploadService.cpp:145-155` も `DownloadService` と同じ「ロック下でコピー → 解放 → 呼び出し」
    であることを実物で確認済み（`UploadController` 側のコメントはこれを前提に書いてある）。
...
- **対応（R6-1、2026-08-08）**: per-delegate 4 箇所のインライン `DropArea` を `NodeDropArea` へ差し替え。
  279 行削除 / 64 行追加。R6-5 が判断を全部前倒しで潰していたので、残っていた決定は「固有の追加を
  どのシグナルに吊るすか」だけだった。実施して分かったこと:
  - **`id` を据え置きにしたのが差分を drop ブロックに閉じ込めた。** 4 箇所とも背景の
    `border.width: dropArea.accepting ? Theme.border.drop : 0` が `accepting` を外から読んでいる。
    `NodeDropArea` が同名で公開しているので、`id: dropArea` / `id: tabDropArea` をそのまま名乗らせれば
    枠線側は 1 文字も変わらない。名前を揃えるついでに改名すると、無関係な行が diff に混ざって
    スクショ差分の切り分けが効かなくなる。
  - **共通化するとコメントの宿主が消える、という副作用がある。** `TabStrip` の
    `onPositionChanged` は中身が `drag.accepted` の再表明だけ＝丸ごと `NodeDropArea.syncMove()` なので
    消える。ところがそこに付いていた 6 行のコメントは「**dwellTimer にわざと触っていない**」という
    否定形の設計意図で、ハンドラごと消すと根拠の置き場が無くなる。`onDragMoved` を**書かない**理由
    として書き直して残した。同じ理由で `QuickAccessSection` の「消えたピンは kENoEnt で拒否」など
    サイト固有の根拠は残し、keys / 非対称 / `drag.accepted` の説明だけを消している。
  - **`TabStrip` の spring-load は `dragEntered` / `dragExited` / `dragDropped` の 3 つに素直に乗った。**
    `dragDropped` が分岐の前に出る仕様（R6-5 の決定）が効くのはここで、インライン時代の
    `onDropped` 先頭の `dwellTimer.stop()` が順序ごと保存される。`FolderTreePanel` の
    `autoScroller.release()` は逆に末尾から先頭へ動くが、R6-5 が観測不能と判断した側。
  - **qmllint の `unqualified` 警告はむしろ減った**（サイトごとに 5〜8 回出ていた `root.dragProxy` が
    1 回の注入になったため）。残る `uploads: uploadController` の 1 件はコンテキストプロパティで、
    リポジトリ全体に既出のカテゴリ。`missing-property` 群（`Theme` が `QObject` 扱い）は元から。
  - **テスト 486 件全通過は差し替えの正しさを保証しない。** `tst_NodeDropArea` が見ているのは
    コンポーネント単体で、4 サイトの配線はテスト外のまま（R6-5 の穴はここまでしか埋まらない）。
    スクショも「起動する＝`required` の渡し忘れが無い」しか言えない。**実ドラッグの手動確認が唯一の
...
- **対応（R6-3、2026-08-08）**: `headerComponent` / `footerComponent` の中身を
  `qml/components/AddressToolBar.qml` / `StatusBar.qml` へ。`Main.qml` は 1071→**590 行**
  （495 行削除 / 14 行追加、`git diff -w` 基準）。新規テストは無し。実施して分かったこと:
  - **「逐語で移すから diff は 1 文字も変わらない」は成立しない。** `.qmlformat.ini` が
    `MaxColumnWidth=100`、かつ `.claude/settings.local.json` の PostToolUse フックが `.qml` の
    Write/Edit ごとに `qmlformat -i` を掛けるので、インデントが 2 段浅くなる移設先では折り返しが
    全部やり直される。R6-1/R6-2 の「据え置きで diff を閉じ込める」戦術がここでは使えない。
    代わりに**コメント除去＋空白正規化してトークン列を突き合わせる**検証をした（意図した置換を
    原典側に適用してから比較）。差分は import・root ヘッダ・後述の修飾・inline component の
    移動だけ、と機械的に言えるので、目視レビューより強い。この形は今後の移設でも使える。
  - **調査の見立て「実質的なコストはインライン component 2 つ」は外れ。** 使用箇所が
    header 4 / footer 2 に**完全分割**されていたので移設先が一意に決まり、判断が要らなかった。
    `qml/components/` に独立ファイル化する案は共有がゼロなので採らず、`CaptionBar.qml` の
    `component CaptionButton` と同じくファイル内に置いた。
  - **inline component に `root.` を書いてはいけない、という制約がここで初めて効いた。** Qt の
    ドキュメントは "Inline components don't share their scope with the component they are
    declared in" と明言していて、`root.` 修飾は**他ファイルから使われた瞬間に** ReferenceError に
    なる。qmllint 対策で機械的に `root.` を足すと踏む。自己参照は `statusButton.` のように
    自分の id で修飾する（原典が既に正しい形だった）。`CaptionBar.qml:164` の
    `root.implicitHeight` は同ファイル内でしか使われていないから助かっているだけで、地雷は残る。
  - **スクショ検証は「映るのに映らない」型だと分かった。** R6-1/2a/2b はドラッグ中しか出ない
    要素で網の外だったが、R6-3 は header/footer が丸ごと映るので light/dark/800x600 の 3 構成で
    最大チャネル差 1/255（アンチエイリアス域）を確認できた。**ただしそれはレイアウトの話**で、
    静かに壊れうる 4 つ — パンくずの `dragProxy` 自己束縛・`currentPane` の書き込み経路・
    ダイアログ 3 signal・`tabsController` の無修飾解決 — は 1 つもピクセル差分に出ない。
    「映るから安全」は誤り。
  - **テストを書かない理由は「スクショに映るから」ではない。** 正しくは**抽出がテストする価値の
    ある注入の継ぎ目を作らないから**。`NodeDropArea`（R6-5）にテストが要求されたのは抽出が
    まさにその継ぎ目を作ったからで、単一インスタンスの移設である R6-3 にはそれが無い。
  - **`window.currentPane` の存在理由コメントが 2 箇所で重複していた**（`Main.qml` の宣言部と
    `Binding` の上）。R6 調査の「コメントの重複はコードの重複に従属する」の実例で、別ファイル化で
    理由が一段強くなったのを機に 1 つに寄せ、もう一方は参照に変えた。
  - **陳腐化コメントは Main.qml の外にもあった。** `Breadcrumb.qml:33-35`（枠高さが検索欄に
    合わせられる配線の在処）と `AccountMenuHeader.qml:26-29`（メニュー幅を明示している側）が
    どちらも "Main.qml" を名指ししており、両方 `AddressToolBar.qml` へ移った。**移設元ファイルの
    grep だけでは見つからない**類で、移設する識別子ではなくファイル名で grep する必要がある。
  - **`qsTr()` の翻訳コンテキストが `Main` から新しい型名へ変わる**（11 文字列）。`.ts`/`.qm` も
    lupdate ターゲットも無いので現時点のコストはゼロ。
  - **実機確認は未実施。** 488/488・QML 警告ゼロ・3 構成のスクショ一致が言えるのは「起動する＝
    `required` の渡し忘れが無い」まで。確認項目: パンくちへのドラッグ&ドロップ（`dragProxy` の
    自己束縛の唯一の検証）/ Back・Up・Refresh の enabled 追従 / 検索の Enter・虫眼鏡・クリア /
...
- **「`header`/`footer` 計 270 行」** → 421 行（R6-3）。実施時の再実測ではスロット込み 439 行、
  専用のインライン `component` 2 つを数えると 518 行。`headerComponent` の終端も 533 ではなく 532、
  `window.currentPane` の参照はコード上 7 箇所（9 は重複カウント）、`window.viewMode` は
  header/footer から**参照ゼロ**（コメント内のみ）だった。
...
### R7 — 対応済み（2026-08-08、R6-1 と R6-2a の間に実施）

- **対応（2026-08-08）**: 2 コミット。`CLAUDE.md` に「Code comments: only what the code can't say」節を
  追加（`2a0789e`）し、その規則で `src/` 全体＋`main.cpp` を掃いた（`be22a5a`、`tests/` は対象外）。
  **コメント行 3049 → 1832、全体で 1063 行減**。コード変更は末尾コメントの短縮 2 箇所だけ。
  消したものは量の多い順に、コードの言い直し／近隣ファイルのコメントを指すだけの相互参照／
  検証と方針の語り（"confirmed against Qt's docs"、"no size cap, this is a one-user app"）／
  フェーズ・R 番号の履歴（コミットが既に持っている）／採らなかった案の長い脚注。残したものは
  外部仕様の罠・もっともらしい「修正」への防御・画面外の力の 3 種類を各 1〜2 行に圧縮した形。
  ついでに `SqliteNodeCache`/`INodeCache` という**この tree に既に無いクラス**への参照が 3 件
  見つかって消えた。

- **実施順は上の「R7 → 最後」に従わなかった。** R6 の途中（R6-1 の後）で入れたのは、R6 調査で
  「コメントの重複はコードの重複に完全に従属している」と分かり（下の持ち越し節）、**C++ 側の掃除は
  QML 側の完了を待つ必要が無い**と判明したため。むしろ先に規則を明文化したほうが、R6-2a 以降の
  各コミットが「触ったファイルはその場で刈る」ルールに乗れる。結果として R6-2a〜R6-4 の新規ファイルは
  最初からこの規則で書かれている。**「最後に書き戻す締め」という当初の想定は、規則の明文化と
  一括掃除を分けた時点で不要になった。**

- **3 つの種はいずれも別経路で解消していた**ので、R7 固有の作業として残らなかった:
  - `FolderNavigationController.h` の「384 行中 250 行超がコメント」→ **174 行 / コメント 71 行**。
    R5-1 の分割で自然に分散し、上の掃除が残りを削った。見立て通り R5 の後に回して正解だった。
  - `CLAUDE.md` の「Project status」が巨大ブロック → 1 段落に縮小済み（`921a427` で done フェーズの
    説明を `docs/PROGRESS.md` の各ログ項目へ折り込んだ）。
  - `FolderNavigationController.h` の「Untested by convention」→ R4 が慣例を書き換えた時点で消滅。
    現在 tree 内に該当文字列は無い。

- **QML 側は独立項目にしなかった**（下の持ち越し節の判断通り）。6 箇所のドロップ受けと 2 ビューの
  コメント重複は R6-1/R6-2 が本体を畳んだ時点で自動的に消えており、R6-3/R6-4 の移設でも
  陳腐化コメントはその場で直している。**残っているコメント負債の見立ては無い。**

- **これで R1–R7 全スコープ完了。** Phase 15 に進める。持ち越し節に残る 3 件は
  スコープ外と判断したもので、積み残しではない（冒頭の注記を見ること）。
