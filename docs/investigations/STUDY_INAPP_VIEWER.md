...
> **状態: 未着手。ただし環境側の前提は 2026-08-29 に確認し直した。** 本文は 2026-08-25 時点の
> コード（`db8357e`）に対する机上調査で、コードは今も変更していない。変わった前提は各節に追記した
> （§2 ビルド設定・§3-3 Qt PDF・§4 WebEngine・§5 呼び出し側）。
> **`docs/ROADMAP.md` の「優先度: 高」に 4 行として載っている** —— §7 の着手順を、土台 / 画像 /
> 動画 / PDF に割ったもの。
...
**2026-08-29 の再確認**: ここは何も変わっていない —— `sdklib_options.cmake:21` は OFF のまま、
`CMakePresets.json:22` の features に `use-libuv` は無いまま、`third_party/vcpkg/ports/libuv` は
実在。つまり上の 2 行を書き足す作業がそのまま残っている。
...
**Qt PDF は 2026-08-29 にインストール済み**（`C:/Qt/6.11.1/msvc2022_64/lib/cmake` に `Qt6Pdf` /
`Qt6PdfQuick` ほか。MSVC 2022 x64 のみ）。調査時点では未インストールで、Maintenance Tool での追加が
前提条件だった。pdfium 自体は vcpkg 経由で SDK が既にビルドしているが、それは SDK 内部の
サムネイル生成用で、こちらから叩ける API ではない。
...
2026-08-29 時点でも未インストールのままで、**ROADMAP では入れないと決めた**。PDF は §3-3 の案 1
（全部メモリに取って渡す）で入れる。
...
**2026-08-29 に呼び出し側を見て分かったこと**、2 点:

- **ファイルのダブルクリックは今なにも起こさない。** `qml/views/TabContentPane.qml` の `activate()`
  はフォルダしか扱わず、コメントによれば「かつて誤爆でダウンロードが走ったので意図的に殺した」もの。
  ビューアはこの空いた動線にそのまま乗るので、新しい操作を考える必要がない。
- **`src/core/PreviewKind.h` は画像・動画・PDF を同じ `Image` 種別に潰している** —— どれもサーバ
  生成の JPEG 1 枚で「プレビュー」できてしまうため。ビューアはこの 3 つを区別するので、この列挙を
  見直すか、ビューア側が独自に判定するかを決める必要がある。
