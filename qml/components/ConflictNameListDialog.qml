...
//
// Also used on its own, with `message` set, to report a copy or move that was
// refused rather than asked about (TabContentPane.qml).
//
// Read-only: when it is opened from a question, that question stays on the dialog
// underneath, so this one only closes back to it rather than repeating the answers.
Dialog {
    id: root

    // [{ name: string, isFolder: bool }], in the order the caller listed them.
    property var entries: []
    // Optional line above the list. Empty for the two dialogs that open this
    // behind their own text.
    ...
    // asks for -- the window less an allowance for the title, the footer, the
    // paddings and the message above it, but never less than three rows, so a
    // short window still gets a list rather than an empty frame. That floor is why
    // the height needs its own clamp as well: when it wins, list + chrome is taller
    // than the window, and only this line stops the frame growing past it (the list
    // scrolls instead).
    //
    // Guarded, unlike the sibling dialogs': one instance is declared inside another
    // Dialog, so these first evaluate while the outer popup still has no window and
    // Overlay.overlay is null. The attached property notifies when the overlay
    // arrives, so the fallbacks only have to be harmless, not right.
    ...
    Column {
        ...
        ListView {
            ...
            implicitWidth: Math.min(400, root.textWidthCap)
            implicitHeight: Math.min(contentHeight, root.maxListHeight)
            // Column doesn't resize what it stacks, and the rows have to span a
            // frame the message may have widened.
            width: Math.max(implicitWidth, messageLabel.width)
            model: root.entries
            clip: true
