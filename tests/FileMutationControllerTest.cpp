...
protected:
    void SetUp() override
    {
        ...
        QObject::connect(mutations.get(),
                         &FileMutationController::moveNameConflict,
                         mutations.get(),
                         [this](QVariantList entries,
                                QStringList files,
                                QStringList folders,
    ...
    // The destination read a copy or a move makes before it issues anything --
    // folder `handle`'s children, independent of what this tab is showing.
    void givenChildrenOf(std::uint64_t handle, std::vector<FileEntry> entries)
    {
        EXPECT_CALL(*client, getChildren(handle, _, _))
            .WillRepeatedly(
                InvokeArgument<2>(Result<std::vector<FileEntry>>::ok(std::move(entries))));
    }
...
TEST_F(FileMutationControllerTest, MoveEntriesToReportsOneTallyForTheWholeDrop)
{
    givenRootListing({entry("a", 1), entry("b", 2), entry("c", 3)});
    controller->loadRoot();
    flush();
    ...
    mutations->moveEntriesTo(
        clipboardEntries({entry("a", 1), entry("b", 2), entry("c", 3)}), 99, false);
    flush();
    flush();
...
TEST_F(FileMutationControllerTest, MoveEntriesToSeparatesSucceededFromFailed)
{
    givenRootListing({entry("a", 1)});
    controller->loadRoot();
    flush();
...
TEST_F(FileMutationControllerTest, MoveEntriesToForwardsTheRootSentinel)
{
...
TEST_F(FileMutationControllerTest, MoveEntriesToDoesNothingWithAnEmptyDrop)
{
    givenRootListing({entry("a", 1)});
    controller->loadRoot();
    flush();
...
TEST_F(FileMutationControllerTest, PasteAsksBeforeCopyingOntoAnExistingFile)
{
    ...
    EXPECT_TRUE(lastConflictFolders.isEmpty());
...
TEST_F(FileMutationControllerTest, PasteAsksAboutAFolderWhoseNameIsTaken)
{
    // Reported under folders, not files: the dialog hides Replace on this one,
    // because copyNode cannot merge a subtree into an existing folder.
    ...
    ASSERT_EQ(conflictCalls, 1);
    EXPECT_TRUE(lastConflictFiles.isEmpty());
...
TEST_F(FileMutationControllerTest, CopyEntriesToOntoTheirOwnFolderDuplicatesWithoutAsking)
{
    // Ctrl+dropping a selection back onto the folder it came from: its own node
    // is what the name clashes with, so it duplicates rather than asks.
    givenRootListing({});
    controller->loadRoot();
    flush();
...
TEST_F(FileMutationControllerTest, MoveEntriesToAsksBeforeLandingOnATakenName)
{
    givenRootListing({entry("a.txt", 1)});
    controller->loadRoot();
    flush();
...
{
    // A file and a folder sharing a name can neither overwrite nor merge into
    // each other, so that pair is no collision -- the same rule the copy path
    // follows.
    givenRootListing({});
    controller->loadRoot();
    flush();
...
TEST_F(FileMutationControllerTest, MoveSkippingExistingMovesOnlyTheEntriesThatDidNotCollide)
{
    givenRootListing({});
    controller->loadRoot();
    flush();
...
TEST_F(FileMutationControllerTest, MoveEntriesToRefusesTwoEntriesThatShareAName)
{
    ...
    mutations->moveEntriesTo(clipboardEntries({entry("a.txt", 1), entry("a.txt", 2)}), 7, false);
    flush();
    flush();
...
{
    // Same rule as against the destination: a file and a folder of one name can
    // neither overwrite nor merge, so they are not each other's collision.
    givenRootListing({});
    controller->loadRoot();
    flush();
    givenChildrenOf(7u, std::vector<FileEntry>{});
    ...
    mutations->moveEntriesTo(clipboardEntries({entry("a", 1), entry("a", 2, true)}), 7, false);
    flush();
    flush();
