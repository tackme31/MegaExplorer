...
TEST(UploadScanServiceTest, AsksAboutEachNameOnceEvenWhenMegaHasSameNamedSiblings)
{
    // MEGA allows same-named siblings, and one upload can carry two files with
    // the same leaf name from different folders: neither may turn one local file
    // into two collision rows, nor one folder into two descents.
    auto fs = std::make_shared<FakeLocalFileSystem>();
