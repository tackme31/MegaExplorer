...
`git` commands in this repo (add/commit/status, etc.) intermittently fail with
`fatal: Unable to create '.../.git/index.lock': File exists` even though no git operation is
actually in progress. Recurs across sessions; root cause not pinned down (candidate: Qt Creator's
own git integration, or a prior command that got interrupted — e.g. a killed tool call — without
cleaning up its lock). Confirmed-safe recovery, used repeatedly:
